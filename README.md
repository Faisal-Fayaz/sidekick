...
| 427-test suite incl. prompt-regression evals | ✅ | rare |
...
| `sk skills` / `sk skills-search` / `sk skills-install superpowers` / `sk plugins` / `sk daemon [--once]` / `sk daemon-install [--schedule TXT]` / `sk daemon-schedule [--set TXT]` | Skill packs (obra/superpowers) / user-defined tools (`TOOLS.md`, see `docs/plugins.md`) / background watcher (systemd/launchd, calendar schedules) |
...
```bash
uv run --python 3.12 --with ".[test]" pytest tests -q   # 427 passed: unit + regression + Textual pilot, no Ollama needed
```
