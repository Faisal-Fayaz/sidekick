...
| Command | What |
|---|---|
| `sk chat` / `sk tui [--mouse]` | Interactive chat (REPL / fullscreen), `/help` inside. TUI keys: Enter sends, ctrl+j/alt+enter newline, ↑/↓ history, ctrl+y copies, ctrl+t push-to-talk with mic status pill, ctrl+b/f page-scroll, ctrl+home/end jump. Answers stream live, thinking dimmed, footer shows last-turn time/tokens. Copy works like a normal terminal (mouse tracking stays off unless `--mouse`); `/copy [n]` uses native clipboard → wl-copy/xclip/xsel → OSC52 (`sudo apt install xclip` on plain X11) |
